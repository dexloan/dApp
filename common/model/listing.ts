...
import { ListingState, LoanData } from "../types";

export type ListingPretty = ReturnType<Listing["pretty"]>;
...
export class Listing implements ListingArgs {
  ...
  get dueDate() {
    if (this.data.state === ListingState.Listed) {
  ...
  static fromJSON(args: ListingPretty) {
    return new Listing(
      {
